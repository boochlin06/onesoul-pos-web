...
import {
  ...
  Users, BarChart3, ClipboardList, Receipt, Search, BookOpen, X, Loader2, ChevronDown, Package, CheckCircle2, Box, Check, MessageSquare, AlertCircle, RefreshCw
} from 'lucide-react';
...
// ── Members View ──────────────────────────────────────────
function MembersView({ members, isLoading, onRefresh }: { members: MemberEntry[], isLoading: boolean, onRefresh: () => void }) {
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex items-center justify-center animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col transform transition-all border border-slate-100 p-6 m-4">
            ...
            <p className="text-slate-500 text-sm mt-3 mb-6 ml-13 leading-relaxed">
              作廢後，該筆**當日交易紀錄**將被刪除，且其產生的點數變化會自動退回給會員。<br/>⚠️ 此操作無法復原。
            </p>
    ...
    gasPost('deleteDailySales', { branch, checkoutUID })
      .then(res => {
        if (res.success) {
          ...
          fetchDailySales(); // Refetch sales
          fetchMembers(); // Refetch members to update local cache with refunded points
        } else {
