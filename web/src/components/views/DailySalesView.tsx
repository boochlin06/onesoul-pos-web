...
import { Search, ClipboardList, Receipt, Loader2, Trash2, Archive, MessageSquare } from 'lucide-react';
import { RefreshButton } from '../ui/RefreshButton';
import { useStickyState } from '../../hooks/useStickyState';
import { Pagination } from '../ui/Pagination';
import { VoidSaleModal } from '../checkout/VoidSaleModal';
  ...
  return (
    <div className="flex flex-col gap-6 mb-24 max-w-screen-2xl mx-auto">
      ...
      {voidConfirmUid && (() => {
        ...
        if (!voidGroup) return null;
        
        return (
          <VoidSaleModal
            uid={voidConfirmUid}
            items={voidGroup.items}
            members={members}
            onConfirm={(uid) => {
              onDelete(uid);
              setVoidConfirmUid(null);
            }}
            onCancel={() => setVoidConfirmUid(null)}
          />
        );
      })()}
    </div>
