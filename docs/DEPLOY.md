...
| `services.js` | 後端業務邏輯 |
| `main.js` | GAS 主程式 |
| `docs/` | 所有技術文件（介紹、操作流程、部署手冊、更新日誌） |
| `wiki/` | GitHub Wiki 頁面來源（自動同步至 Wiki） |
| `.clasp.json` | clasp 對應的 GAS 專案設定 |
| `.github/workflows/` | CI/CD（前端部署、Wiki 同步） |
...
| 措施 | 說明 |
|------|------|
| ✅ Google OAuth | 員工 POS 操作需要 Google 帳號登入，後端 JWT 解碼驗證 iss + exp + aud |
| ✅ email 白名單 | 只有 `ALLOWED_EMAILS` 裡的帳號才能操作 |
| ✅ API Key 驗證 | 所有 API 請求都需要正確的金鑰 |
| ✅ LockService | 防止多人同時寫入造成資料覆蓋 |
| ✅ 原子化點數 | `addMemberPointsByPhone` 鎖內讀 DB 即時值 + 相對加減，防 race condition |
| ✅ 速率限制 | 客戶登入同電話 60s/5次，防暴力破解 |
| ✅ deleteRow 保護 | 批量刪除顯式 sort(b-a)，防行號偏移 |
...
| 🟡 客戶頁面無密碼 | 低 | 會員登入只靠手機號碼，知道號碼就能查到該會員的點數（但無法修改），且有速率限制保護 |
